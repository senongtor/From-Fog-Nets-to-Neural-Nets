...
def main():
    ...
    for k in xrange(file_amount):
        ...
        X_train, X_test, y_train, y_test, train_data, test_data = \
            split_and_build_class(df.values, yield_df.values)
        ...
        plt.suptitle('Cross Valication + SVM Binary Classification + Ridge Regression')
        plt.savefig('Cross Valication + SVM Binary Classification + Ridge Regression.png', bbox_inches='tight')

        loss = np.sqrt(mean_squared_error(y_test, y_hat_test))
        print 'Cross Valication + SVM Binary Classification + Ridge Regression loss =', loss
        ...
        y_submission = write_submission.write_submission_binary_classifier_and_regression(
            X_combined, clf, clf_regression, df_submission, 'Cross Valication + SVM Binary Classification + Ridge Regression Submission')
